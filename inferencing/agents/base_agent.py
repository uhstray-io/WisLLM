...
class BaseAgent(ABC):
    ...
    async def handoff_to_agent(self, target_agent: str, payload: Dict[str, Any]) -> None:
        """Handoff task to another agent"""
        await self.handoff_manager.handoff(from_agent=self.agent_id, to_agent=target_agent, payload=payload)
